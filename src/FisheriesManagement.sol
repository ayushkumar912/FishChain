...
contract FisheriesManagement {
    ...
    event FishLogged(uint256 batchId, address fisher, uint256 weight, uint256 pricePerKg);
    ...
    function logCatch(uint256 weight, uint256 pricePerKg) public {
        batches[nextBatchId] = FishBatch(nextBatchId, msg.sender, weight, pricePerKg, false, false, false, new uint256[](0));
        emit FishLogged(nextBatchId, msg.sender, weight, pricePerKg);
        nextBatchId++;
    }

    function updateSustainability(uint256 batchId, bool sustainable) public onlyGovernment {
        batches[batchId].sustainable = sustainable;
        emit SustainabilityUpdated(batchId, sustainable);
    }

    function raiseDispute(uint256 batchId, string memory reason) public onlyGovernment {
        batches[batchId].inDispute = true;
        emit DisputeRaised(batchId, reason);
    }

    function getBatchSustainability(uint256 batchId) external view returns (bool) {
